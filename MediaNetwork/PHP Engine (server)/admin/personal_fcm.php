    ...
    if (!empty($_POST)) {
        ...
        if ($authToken === helper::getAuthenticityToken() && !APP_DEMO) {
            ...
            if (strlen($message) != 0) {
                ...
                $fcm = new fcm($dbo, $accountId);
                $fcm->setData($fcmId, $type, $message, $addon, 0);
                $fcm->send();
            }
        }

        header("Location: /admin/personal_fcm.php/?id=".$accountId);
    }
            ...
            <div class="card">
                <div class="card-content">
                    <div class="row">
                        ...
                        <div class="row">
                            <div class="col s6">
                                <h4>Send message (FCM) for <a href="/admin/profile.php/?id=<?php echo $accountInfo['id']; ?>"><?php echo $accountInfo['username']; ?></a></h4>
                            </div>
                        </div>
                                        ...
                                        <div class="col s12">
                                            <div class="card blue-grey lighten-2">
                                                <div class="card-content white-text">
                                                    <span class="card-title">Sending push notifications (FCM) is not available in the demo version mode. That we turned off the sending push notifications (GCM) in the demo version mode to protect users from spam and of foul language.</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <form method="post" action="/admin/personal_fcm.php/?id=<?php echo $accountId; ?>">
                            ...
                            <div class="row">
                                ...
                                <div class="input-field col s12">
                                    <select name="type">
                                        <option selected="selected" value="<?php echo GCM_MESSAGE_FOR_ALL_USERS; ?>">It will be shown, even if the user is not authorized</option>
                                        <option value="<?php echo GCM_MESSAGE_ONLY_FOR_AUTHORIZED_USERS; ?>">Only an authorized user</option>
                                    </select>
